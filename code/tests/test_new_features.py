...
class TestMultiTokenPrediction(unittest.TestCase):
    ...
    def test_mtp_head_creation(self):
        """Test MTP head can be created with default config."""
        from ava.models.mtp_head import MTPConfig, MultiTokenPredictionHead
        ...
        mtp = MultiTokenPredictionHead(config).to(self.device)

        # Primary head + (num_prediction_heads - 1) additional heads
        self.assertEqual(len(mtp.additional_heads), 3)
        self.assertEqual(len(mtp.confidence_gates), 3)

    def test_mtp_head_forward(self):
        """Test MTP head forward pass."""
        from ava.models.mtp_head import MTPConfig, MultiTokenPredictionHead
        ...
        result = mtp(hidden_states, training=False)

        # Check output - forward returns a dict
        self.assertIn('predictions', result)
        self.assertIn('confidences', result)
        # Primary head + (num_heads - 1) additional = num_heads predictions total
        self.assertEqual(len(result['predictions']), 3)
        # All heads get confidence scores (primary gets 1.0, additional get learned)
        self.assertEqual(len(result['confidences']), 3)
...
class TestMixtureOfHeads(unittest.TestCase):
    """Tests for Mixture of Heads module."""
    ...
    def test_moh_creation(self):
        """Test MoH can be created."""
        from ava.models.moh import MoHConfig, MixtureOfHeads
        ...
        moh = MixtureOfHeads(config).to(self.device)

        self.assertEqual(moh.num_heads, self.num_heads)
        self.assertEqual(moh.num_active, 4)

    def test_moh_forward(self):
        """Test MoH forward pass."""
        from ava.models.moh import MoHConfig, MixtureOfHeads
        ...
        q = k = v = torch.randn(self.batch_size, self.seq_len, self.hidden_size, device=self.device)
        output, aux_loss, info = moh(q, k, v, training=True)

        self.assertEqual(output.shape, (self.batch_size, self.seq_len, self.hidden_size))
        self.assertIsInstance(aux_loss, torch.Tensor)
        self.assertIn('load_balance_loss', info)

    def test_head_router(self):
        """Test head router selection."""
        from ava.models.moh import HeadRouter
        ...
        hidden_states = torch.randn(self.batch_size, self.seq_len, self.hidden_size, device=self.device)
        weights, indices, info = router(hidden_states, training=True)

        self.assertEqual(weights.shape, (self.batch_size, self.seq_len, 4))
        self.assertEqual(indices.shape, (self.batch_size, self.seq_len, 4))
        # Verify top-k selection
        self.assertTrue((indices >= 0).all() and (indices < self.num_heads).all())


class TestMixtureOfActivations(unittest.TestCase):
    """Tests for Mixture of Activations module."""
    ...
    def test_moa_creation(self):
        """Test MoA can be created."""
        from ava.models.moa import MoAConfig, MixtureOfActivations
        ...
        moa = MixtureOfActivations(config).to(self.device)

        self.assertEqual(len(moa.activations), 4)

    def test_moa_forward(self):
        """Test MoA forward pass."""
        from ava.models.moa import MoAConfig, MixtureOfActivations
        ...
        hidden_states = torch.randn(self.batch_size, self.seq_len, self.hidden_size, device=self.device)
        output, aux_loss, info = moa(hidden_states, training=True)

        self.assertEqual(output.shape, (self.batch_size, self.seq_len, self.hidden_size))
        self.assertIsInstance(aux_loss, torch.Tensor)
        self.assertIn('activation_usage', info)

    def test_activation_registry(self):
        """Test activation registry has expected functions."""
        from ava.models.moa import ACTIVATION_REGISTRY

        expected_activations = ['gelu', 'relu', 'silu', 'tanh', 'mish']
        for act_name in expected_activations:
            self.assertIn(act_name, ACTIVATION_REGISTRY)
            # Test the activation works
            x = torch.randn(10)
            result = ACTIVATION_REGISTRY[act_name](x)
...
def run_tests():
    ...
    test_classes = [
        TestMultiTokenPrediction,
        TestMixtureOfHeads,
        TestMixtureOfActivations,
        TestALiBi,
        TestCrossAttention,

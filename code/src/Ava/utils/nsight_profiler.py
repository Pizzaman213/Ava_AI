...
Usage:
    ...
    from src.Ava.utils.nsight_profiler import NsightProfiler, DetailedProfiler
...
try:
    from Ava.utils.cuda_streams import efficient_sync, optimal_sync_context
    ...
    def add_observation(self, tensor: torch.Tensor, compute_stats: bool = False):
        ...
        if compute_stats and tensor.numel() > 0:
            with torch.no_grad():
                ...
                self.min_vals.append(flat.min().item())
                self.max_vals.append(flat.max().item())
                self.mean_vals.append(flat.mean().item())
                self.std_vals.append(flat.std().item())
                self.num_nans.append(torch.isnan(tensor).sum().item())
                self.num_infs.append(torch.isinf(tensor).sum().item())

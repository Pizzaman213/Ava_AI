...
from ..utils.async_prefetcher import AsyncBatchPrefetcher
from ..utils.gradient_utils import check_gradients
...
try:
    from Ava.utils.nsight_profiler import NsightProfiler
...
class TrainingLoopManager(ManagerInterface):
    ...
    def train_epoch(
        ...
        try:
            for batch_idx, gpu_batch in self.prefetcher:
                try:
                    ...
                    if is_log_step:
                        ...
                        self._log_step_metrics(
                            batch_idx, gpu_batch, last_synced_loss, optimizer, config, epoch
                        )
                    ...
                    if is_log_step:
                        new_total = self.prefetcher.get_dynamic_total()
                        if pbar.total != new_total:
                            pbar.total = new_total

                        pbar.set_postfix({
                            'loss': f'{last_synced_loss:.4f}',
                            'BS': gpu_batch['input_ids'].shape[0]
                        })
                    ...
                    batch_controller = getattr(self._context, 'batch_controller', None)
                    if batch_controller is not None:
                        ...
                        self.logger.warning(
                            f"OOM at batch {batch_idx} (BS={current_bs}), "
                            f"reducing to BS={new_size}. Retrying..."
                        )
                        ...
                        self.logger.error(
                            f"OOM at batch {batch_idx}: {oom_error}. "
                            f"No batch controller available. "
    ...
    def _log_step_metrics(
        ...
        if is_main:
            ...
            self.logger.info(
                f"Epoch {epoch + 1} | Step {self._global_step}/{total} | "
    ...
    def setup_profiler(self, config: TrainingLoopConfig) -> None:
        ...
        from Ava.utils.nsight_profiler import NsightProfiler, enable_nsight_profiling

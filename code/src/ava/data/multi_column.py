...
# Set up logging (avoid basicConfig to prevent duplicate handlers)
...
class MultiColumnDataset(Dataset):
    ...
    def _process_video(self, video_data: Any, config: ColumnConfig) -> torch.Tensor:
        ...
        if conv3d_format:
            ...
            return torch.zeros(channels, num_frames, height, width)
        else:
            # [T, C, H, W] for frame-by-frame processing
            return torch.zeros(num_frames, channels, height, width)

...
Usage:
    from ava.logging.wandb.wandb import WandBLogger, MetricsManager, WANDB_AVAILABLE
...
if TYPE_CHECKING:
    ...
    from ava.logging.diagnostics.training import LayerGradientStats, ExpertRoutingStats, MemoryBreakdown, TimingProfile
...
class MetricsManager:
    ...
    def _should_log_wandb(self, log_type: str) -> bool:
        ...
        # Check nested wandb config: logging.wandb.log_{type}
        wandb_config = self._logging_config.get('wandb', {})
        log_key = f'log_{log_type}'
        return wandb_config.get(log_key, True)
    ...
    def _should_log_tensorboard(self, log_type: str) -> bool:
        ...
        # Check nested tensorboard config: logging.tensorboard.log_{type}
        tb_config = self._logging_config.get('tensorboard', {})
        log_key = f'log_{log_type}'
        return tb_config.get(log_key, True)
    ...
    def setup(
        ...
        # Setup TensorBoard writer (check nested logging.tensorboard.enabled)
        tb_config = self._logging_config.get('tensorboard', {})
        tensorboard_enabled = tb_config.get('enabled', False)  # Default False - opt-in
        ...
        # Setup WandB if requested and available (check nested logging.wandb.enabled)
        wb_config = self._logging_config.get('wandb', {})
        wandb_config_enabled = wb_config.get('enabled', True)
    ...
    def log_expert_metrics(self, step: int, expert_metrics: Dict[str, float]) -> None:
        """Log per-expert metrics (utilization, load, capacity).

        These metrics are logged under the 'experts/' prefix for clear separation
        from aggregated MoE metrics.

        Args:
            step: Current training step
            expert_metrics: Dictionary of expert metrics. Keys should be formatted as:
                - 'expert_{id}_utilization': Per-expert utilization
                - 'expert_{id}_load': Per-expert token load
                - 'expert_{id}_capacity': Per-expert capacity usage
                - 'avg_utilization': Average utilization across experts
                - 'avg_load': Average load across experts
                - 'util_{key}_l{layer}': Per-layer utilization breakdown
        """
        if not self._should_log('expert'):
            return

        if self._should_log_tensorboard('experts'):
            for key, value in expert_metrics.items():
                self._writer.add_scalar(f'experts/{key}', value, step)

        if self._should_log_wandb('experts'):
            log_dict = {f'experts/{key}': value for key, value in expert_metrics.items()}
            self.accumulate_wandb_metrics(log_dict, step)
    ...
    def log_routing_diagnostics(self, step: int, routing_stats: 'ExpertRoutingStats') -> None:
        ...
        Routing-level metrics (balance, entropy, dropped tokens) go under 'routing/'.
        Per-expert metrics (load, capacity) go under 'experts/' for separation.
        """
        if not self._logging_config.get('wandb_log_routing_diagnostics', False):
            return

        # Routing-level metrics stay in routing/ section
        routing_metrics = {
        ...
        # Per-expert metrics go to experts/ section
        expert_metrics = {}
        total_load = 0.0
        total_capacity = 0.0
        num_experts = 0
        ...
        for expert_id, load in routing_stats.per_expert_load.items():
            expert_metrics[f'experts/expert_{expert_id}_load'] = load
            total_load += load
            num_experts += 1

        for expert_id, usage in routing_stats.capacity_usage.items():
            expert_metrics[f'experts/expert_{expert_id}_capacity'] = usage
            total_capacity += usage

        # Add summary statistics for experts
        if num_experts > 0:
            expert_metrics['experts/avg_load'] = total_load / num_experts
            expert_metrics['experts/avg_capacity'] = total_capacity / num_experts

        # Combine all metrics for logging
        all_metrics = {**routing_metrics, **expert_metrics}
        ...
        if self._use_wandb:
            self.accumulate_wandb_metrics(all_metrics, step)

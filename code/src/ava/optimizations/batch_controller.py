...
class BatchSizeController:
    ...
    def startup_calibration(
        ...
        optimizer_memory_factor: float = 0.6,
    ) -> int:
        """
        Binary search for optimal batch size at training start.

        IMPORTANT: This now runs in training mode with actual backward passes
        to accurately estimate memory usage including gradients and activations.

        OPTIMIZER MEMORY: Calibration runs BEFORE the optimizer is created, but
        Adam-style optimizers add ~2x model size for momentum and variance states.
        The optimizer_memory_factor (default 0.6) reserves headroom for this:
        - Model + gradients during calibration: ~2x model size
        - Optimizer states added later: ~2x model size
        - Total during training: ~4x model size
        - Factor of 0.6 means: calibrate to 60% of target, leaving 40% for optimizer
        ...
        Args:
            ...
            use_training_mode: If True, run forward+backward for accurate memory estimation
            optimizer_memory_factor: Factor to reduce target memory to account for
                optimizer states (default 0.6 = reserve 40% for Adam momentum/variance)
        ...
        # Apply optimizer memory factor to reserve headroom for optimizer states
        # This is critical because calibration runs before optimizer is created
        effective_target = target_memory * optimizer_memory_factor

        start_time = time.time()
        logger.info(
            f"Starting batch size calibration (target={target_memory:.0%}, "
            f"effective={effective_target:.0%} after optimizer factor={optimizer_memory_factor}, "
            f"timeout={max_time_seconds}s, training_mode={use_training_mode})"
        )
        ...
        while test_size <= self._max_batch_size:
            ...
            try:
                ...
                if use_training_mode:
                    # Full forward + backward pass for accurate memory estimation
                    # This accounts for gradients and activation memory
                    ...
                    loss.backward()
                    model.zero_grad(set_to_none=True)  # Clear gradients after measuring
                else:
                    with torch.no_grad():
                        _ = model(
                            input_ids=batch.get('input_ids'),
                ...
                logger.info(f"Calibration: BS={test_size}, util={util:.1%} (target={effective_target:.0%})")

                if util > effective_target:
                    # Over effective target (with optimizer headroom), but not OOM
                    first_bad = test_size
                    logger.info(f"Found upper bound at BS={test_size} (util={util:.1%} > {effective_target:.0%})")
                    break
                else:
                    last_good = test_size
        ...
        while first_bad - last_good > self._min_batch_size:
            ...
            try:
                ...
                util = self._memory_monitor.get_utilization(force_refresh=True)
                logger.info(f"Binary search: BS={mid}, util={util:.1%} (target={effective_target:.0%})")

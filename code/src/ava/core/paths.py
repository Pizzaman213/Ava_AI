...
Usage:
    from ava.core.paths import get_project_root, get_data_dir, get_models_dir
...
def get_project_root() -> Path:
    ...
    # code/src/ava/core/paths.py -> up 5 levels -> /root/Ava_AI
    if current.parts[-5:-1] == ("code", "src", "ava", "core"):
...
def get_tokenizer_path(tokenizer_name: str = "enhanced-50680") -> Path:
    ...
    1. code/models/tokenizer/{tokenizer_name}
    2. code/data/Ava_Ai/tokenizer (common location for pre-trained tokenizer)
    3. Falls back to first path if none exist
    ...
    Examples:
        >>> get_tokenizer_path()  # Returns first existing tokenizer path
        >>> get_tokenizer_path("enhanced-65536")  # Returns code/models/tokenizer/enhanced-65536
    """
    # Primary location: code/models/tokenizer/{name}
    primary_path = get_models_dir() / tokenizer_name
    if primary_path.exists():
        return primary_path

    # Secondary location: code/data/Ava_Ai/tokenizer
    secondary_path = get_code_dir() / "data" / "Ava_Ai" / "tokenizer"
    if secondary_path.exists():
        return secondary_path

    # Fallback to primary path (let downstream handle missing)
    return primary_path
...
def resolve_path(relative_path: str, base: Optional[Path] = None) -> Path:
    ...
    Examples:
        ...
        Path('/root/Ava_AI/code/data/processed')

        >>> resolve_path("file.txt", get_outputs_dir())
        Path('/root/Ava_AI/code/outputs/file.txt')
    """
    if base is None:
...
def add_src_to_path():
    ...
    Examples:
        >>> add_src_to_path()
        >>> from ava.core import paths  # Now this works from anywhere
    """

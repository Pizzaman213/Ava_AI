...
try:
    from ava.core.paths import get_outputs_dir
except ImportError:
...
class RunManager:
    ...
    def _setup_logging(self):
        ...
        for logger_name, config in log_configs.items():
            ...
            # Console handler for training and errors (with colors if available)
            if logger_name in ['training', 'errors']:
                console_handler = logging.StreamHandler()
                console_handler.setLevel(config['level'])
                # Use colored formatter if available, otherwise plain text
                if ColoredFormatter is not None:
                    console_formatter = ColoredFormatter(show_level=True, show_icons=True)
                else:

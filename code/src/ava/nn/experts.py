...
try:
    from ..kernels.activations import (
        fused_swiglu,
        fused_geglu,
...
try:
    from ..kernels.fused_experts import (
...
class ExpertParallelGroup(nn.Module):
    ...
    def forward(
        ...
        1. Async Pipeline (DEFAULT, FASTEST): Multi-stream parallel expert processing
        2. Loop experts: Loops over experts, minimal D2D copies
        3. Fused Triton: Single kernel for entire expert computation
        4. Grouped GEMM: index_select + bmm, moderate D2D
        5. Sparse dispatch: Similar to grouped GEMM
        ...
        Args:
            ...
            use_async_pipeline: If True, use async multi-stream pipelining (BEST: 34x speedup)

        Returns:
            Expert outputs [num_tokens, k, hidden_size]

        Performance comparison (E=32, k=2, N=4096, RTX 3090 Ti):
        - Async Pipeline: 8.4ms - 34x speedup (DEFAULT)
        ...
        # OPTIMIZATION: Try async pipelining FIRST (fastest: 34x speedup)

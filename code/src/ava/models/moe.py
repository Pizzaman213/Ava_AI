...
class EnhancedMoEConfig:
    ...
    # COHERENCE FIX: Reduced defaults to prevent aux losses dominating CE loss
    router_z_loss_coef: float = 0.0001  # Router z-loss for stability (reduced 10x for coherence)
    load_balance_loss_coef: float = 0.001  # Load balancing loss (reduced 10x for coherence)
    ...
    # Loss regularization features for coherence (COHERENCE FIX: safer defaults)
    entropy_regularization: float = 0.001  # Entropy bonus for diverse predictions (reduced for stability)
    output_diversity_weight: float = 0.0  # DISABLED by default - can cause incoherent outputs if too high
    ...
    label_smoothing: float = 0.1  # COHERENCE FIX: Enabled by default - prevents overfitting, improves coherence

    # Weight tying
    tie_word_embeddings: bool = True  # COHERENCE FIX: Enabled by default - improves vocab learning, reduces parameters
